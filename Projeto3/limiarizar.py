...
def main():
    ...
    parser.add_argument('--size', help='Tamanho da janela para limiarizacao local (default eh 7, para uma janela 7x7).', default=7, type=int)
    parser.add_argument('--k', help='Valor que pode ser definido para o parametro "k". Valor default 0.25', default=0.25, type=float)
    parser.add_argument('--r', help='Valor que pode ser definido para o parametro "r". Valor default 0.5', default=0.5, type=float)
    ...
    # Aux para ver a imagem que esta sendo lida na entrada
    show_image(inputImg, None)

    if(args.methodName == "global"):
        img = global_thresholding(img, args.thresh)
        # Aux para ver a imagem que esta sendo enviada para saida
        show_image(img, None)
        # argumentos: img de saida, img original, salvar histograma, nome da img, pasta de saida
        histograma_e_pixels(img, inputImg, save=True, name="global_"+basename(args.imgPath), folder="Outputs")
        save_image(img, args.imgPath, "global", "Outputs")
    else:
        ...
        show_image(img, None)
        # argumentos: img de saida, img original, salvar histograma, nome da img, pasta de saida
        histograma_e_pixels(img, inputImg, save=True, name="local_"+ args.methodName +"_"+basename(args.imgPath), folder="Outputs")
        save_image(img, args.imgPath, "local_"+ args.methodName, "Outputs")
...
def local_thresholding(img, methodName, size, k, r, p, q):
    ...
    for y in range(img.shape[0]):
        for x in range(img.shape[1]):
            janela_metodo_local = img[max(0, y-deltaXY) : min(y+deltaXY+1, img.shape[0]),
                                      max(0, x-deltaXY) : min(x+deltaXY+1, img.shape[1])]
            
            if(methodName == "bernsen"):
                t_mat[y,x] = bernsen(janela_metodo_local, k, r, p, q)
            elif(methodName == "niblack"):
                t_mat[y,x] = niblack(janela_metodo_local, k, r, p, q)
            elif(methodName == "sauvola"):
                t_mat[y,x] = sauvola_pietaksinen(janela_metodo_local, k, r, p, q)
            elif(methodName == "more"):
                t_mat[y,x] = phansalskar_more_sabale(janela_metodo_local, k, r, p, q)
            elif(methodName == "contrast"):
                t_mat[y,x] = contrast_method(janela_metodo_local, k, r, p, q)
            elif(methodName == "mean"):
                t_mat[y,x] = mean_method(janela_metodo_local, k, r, p, q)
            elif(methodName == "median"):
                t_mat[y,x] = median_method(janela_metodo_local, k, r, p, q)
            else:
...
def sauvola_pietaksinen(win, k, r, *_):
    return np.mean(win) * (1 + k*(np.std(win)/r - 1))

# Metodo Local de thresholding projetado sobre Sauvola e Pietaksinen, para imgens de baixo contraste.
# k=0.25, r=0.5, p=2 and q=10 sao valores sugeridos.
def phansalskar_more_sabale(win, k, r, p, q):
    mean = np.mean(win)
    return mean * (1 + p * exp(-1*q*mean) + k * (np.std(win)/r - 1))

# Metodo Local de thresholding para checagem de contraste.
def contrast_method(win, *_):
    shape = win.shape
    pixel = win[shape[0]//2, shape[1]//2]
    return 0 if abs(int(pixel)-int(np.amax(win))) < abs(int(pixel)-int(np.amin(win))) else 255

# Metodo Local de thresholding: mean
def mean_method(win, *_):
    return np.mean(win)

# Metodo Local de thresholding: median
def median_method(win, *_):
    return np.median(win)

# Metodo auxiliar para verificar imagem de entrada e saida
...
def histograma_e_pixels(img, inputImg, save=True, name='image', folder='Outputs'):
    ...
    # Salva histograma da img como um arquivo de saida
    if save:
        f,_ = splitext(name)
        name = "histograma_" + f + '.png'
        path = join(folder,name)
        plt.savefig(path)
    
    # Retorna proporcoes de pixels brancos e pretos para imagem de saida
    qtd_pixels = img.size
    brancos = np.count_nonzero(img)
    pretos = qtd_pixels - brancos
    print()
    print("Fracao de pixels brancos: ", brancos/qtd_pixels)
    print("Fracao de pixels pretos: ", pretos/qtd_pixels)
